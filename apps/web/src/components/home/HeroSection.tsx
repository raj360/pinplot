...
export function HeroSection() {
  const { ready, viewer, countriesByCode, formatListingMoney } =
    useViewerContext();
  ...
  const unlockFee = ready
    ? formatListingMoney(PRICING.tenantUnlockFeeUgx, "UGX", "UG")
    : null;

  const unlockCopy = unlockFee
    ? unlockFee.footnote
    ...
    <section className="grid items-center gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(300px,440px)] lg:gap-10">
      <div className="max-w-xl">
        <p className="mb-2 inline-flex items-center gap-1.5 text-sm font-medium uppercase tracking-wide text-muted">
          ...
          {ready ? `Rentals · ${countryName}` : "Map-first rentals"}
        </p>
        ...
        <p className="mt-4 text-base leading-relaxed text-muted">
          ...
          {ready && !isUganda ? `, ${countryName}` : ""}. Unlock the exact
          location and landlord contact for{" "}
